...
export class RoomsComponent implements OnInit {
  ...
  private getRoomsCategory(id : number) {


    this.roomsSubscription = this.roomsService.listRoomCategory(id)
      .subscribe(data => {
  ...
  filterRooms(roomCategoryId) {
    if (roomCategoryId != -1)
      this.getRoomsCategory(roomCategoryId);
  ...
  showRoomDetail(room: Room) {
    if(room.room_status_id != 1){
      ...
      this.alert.warning("Habitación no disponible para reservar");
      return;
    }
    console.log(room);
