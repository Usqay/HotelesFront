...
export class RoomsService {
  ...
  listRoomCategory(id = 0) :  Observable<ApiResource>{
    let url = `${environment.apiUrl}rooms/list?id=${id}`
  ...
  getData(){
    return this.http.get(`${environment.apiUrl}room-reserve/data`,{
      headers : this.headers
    });
  }
  getCurrencPaymentMethod(){
    return this.http.get(`${environment.apiUrl}getCurrencPaymentMethod`,{
      headers : this.headers
    });
  }
